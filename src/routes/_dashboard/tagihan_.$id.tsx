...
import { getTagihanSiswaDetail } from '#/server/tagihan'
import { getYayasanPengaturan } from '#/server/pengaturan'
import { useState, useEffect, lazy, Suspense } from 'react'
import { ArrowLeft, FileText, Eye } from 'lucide-react'
import { TagihanKwitansiPDF } from '#/components/tagihan-kwitansi-pdf'
const PDFViewer = lazy(() =>
  import('@react-pdf/renderer').then((mod) => ({ default: mod.PDFViewer })),
)

export const Route = createFileRoute('/_dashboard/tagihan_/$id')({
  beforeLoad: async () => {
...
function TagihanDetailPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      getTagihanSiswaDetail({ data: { id } }),
      getYayasanPengaturan(),
    ])
      .then(([td, pg]) => { setData(td); setPengaturan(pg) })
  ...
  if (error) return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <Link to="/tagihan" search={{ s: undefined }} className="p-2 bg-card border-2 border-nb-ink rounded hover:bg-muted-foreground/10 cursor-pointer">
          <ArrowLeft className="w-4 h-4" />
        </Link>
        <h2 className="font-heading font-bold text-lg">Detail Tagihan</h2>
      </div>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-heading font-bold text-lg">Detail Tagihan</h2>
            <p className="text-sm text-muted-foreground mt-0.5">{t.siswa?.nama || '-'} — {t.tagihan?.judul || t.tagihan?.tahunAjaran}</p>
          </div>
        </div>
      </div>
          ...
          <div className="bg-card border-2 border-nb-ink rounded p-4">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              <div>
                ...
                <Link to="/siswa/$id" params={{ id: t.siswa?.id }} className="font-heading font-bold mt-0.5 text-primary underline hover:no-underline inline-block">
                  {t.siswa?.nama || '-'}
                </Link>
              ...
              <div>
                <p className="text-sm font-heading font-bold uppercase tracking-wider text-muted-foreground">Tagihan</p>
                <p className="font-heading font-bold mt-0.5">{t.tagihan?.judul || t.tagihan?.tahunAjaran || '-'}</p>
              </div>
              <div>
            ...
            <div className="bg-card border-2 border-nb-ink rounded">
              ...
              <div className="overflow-x-auto">
                <table className="nb-table">
                  ...
                  <tbody>
                    {t.tagihan.items.map((it: any, i: number) => (
                      <tr key={it.id || i}>
                        <td className="text-sm font-heading font-bold">{it.nama}</td>
                        <td className="text-center text-sm">{it.qty}</td>
                        <td className="text-right text-sm">Rp{it.hargaSatuan.toLocaleString('id-ID')}</td>
                        <td className="text-right text-sm text-rose-600">{it.diskon > 0 ? `Rp${it.diskon.toLocaleString('id-ID')}` : '-'}</td>
                        <td className="text-right text-sm font-heading font-bold">Rp{it.subtotal.toLocaleString('id-ID')}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      {tab === 'kwitansi' && (
        <div className="bg-card border-2 border-nb-ink rounded" style={{ height: '90dvh' }}>
          <div className="p-3 border-b-2 border-nb-ink flex items-center justify-between shrink-0">
            <h3 className="font-heading font-bold text-sm">Kuitansi Pembayaran</h3>
          ...
          <div className="flex-1 min-h-0 relative" style={{ height: 'calc(90dvh - 45px)' }}>
            <Suspense fallback={
              ...
              <PDFViewer width="100%" height="100%" showToolbar={true} style={{ border: 'none' }}>
                <TagihanKwitansiPDF data={t} pengaturan={pengaturan || undefined} />
              </PDFViewer>
            </Suspense>
          </div>
        </div>
